...
/*
* importaciones para crear archivos excel
*/
...
class ContabilidadController extends AbstractController
{
    ...
    public function guardarBonos(Request $request)
    {
        ...
        $beneficiario = $bd->getRepository(User::class)->find($request->get('beneficiarioBono'));
    ...
    public function generarInformes(Request $request)
    {
        $bd = $this->getDoctrine()->getManager();

        $tipoInforme = $request->get('tipoInforme');
        $desdeInforme = $request->get('desdeInforme');
        $hastaInforme = $request->get('hastaInforme');
        $servicioInforme = $request->get('servicioInforme');
        $tipoGastoInforme = $request->get('tipoGastoInforme');
        $empresaInforme = $request->get('empresaInforme');
        ...
        if($tipoInforme == 1)
        {
            ...
            if($servicioInforme != '')
            { 
                if($servicioInforme == '3')
                {
                    $query .= " and d.producto_id is not null"; 
                }
                else
                {
                    if($servicioInforme == '1')
                    {
                        $query .= " and st.id in (1,3)"; 
                    }
                    else
                    {
                        $query .= " and st.id = $servicioInforme"; 
                    }
                }
            }

            //if($empresaInforme != ''){ $query .= " and c.id = $empresaInforme"; }

            $data = $bd->getRepository(Movimientos::class)->dataInforme1($desdeInforme, $hastaInforme, $query);
        ...
        elseif($tipoInforme == 2)
        {
            ...
            $data = $bd->getRepository(Movimientos::class)->dataInforme2($desdeInforme, $hastaInforme);
        ...
        elseif($tipoInforme == 3)
        {
            ...
            if($tipoGastoInforme != ''){ $query = " and tipo = $tipoGastoInforme"; }

            $data = $bd->getRepository(Movimientos::class)->dataInforme3($desdeInforme, $hastaInforme, $query);

            foreach ($data as $key => $dato)
            {
                if($dato['tipo'] == 1)
                {
                    $data[$key]['tipo'] = 'Bienes activos fijos';
                }
                elseif($dato['tipo'] == 2)
                {
                    $data[$key]['tipo'] = 'Bienes consumo';
                }
                elseif($dato['tipo'] == 3)
                {
                    $data[$key]['tipo'] = 'Nómina';
                }
                elseif($dato['tipo'] == 4)
                {
                    $data[$key]['tipo'] = 'Servicios públicos';
                }
            }
        ...
        elseif($tipoInforme == 4)
        {
            ...
            $data = $bd->getRepository(Movimientos::class)->dataInforme4($desdeInforme, $hastaInforme, $query);
        ...
        return new JsonResponse(['data' => $data, 'tipoInforme' => $tipoInforme]);
    }

    public function descargarInformes(Request $request)
    {
        $bd = $this->getDoctrine()->getManager();
        ...
        if($tipoInforme == 1)
        {
            ...
            else if ($servicioInforme == '3') {
                $servicio = ' - TIENDA';
            }
            else if ($servicioInforme == '4') {
                $servicio = ' - LAVANDERÍA';
            }
            ...
            $sheet->setCellValue('B1', 'INFORME DE ENTRADAS CONSOLIDADO'.$servicio);
            ...
            $sheet->setCellValue('B2', 'Desde '.$desdeInforme.' | Hasta '.$hastaInforme);
            $sheet->mergeCells('B3:C3');

            $sheet->getColumnDimension('B')->setWidth(20);
            $sheet->getColumnDimension('C')->setWidth(30);

            $sheet->getStyle('B4:C4')->getFont()->setBold(true)->setSize(12);
            $sheet->getStyle('B4:C4')->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);
            ...
            $sheet->getStyle('B4:C4')->applyFromArray($borderStyle);
            $sheet->getStyle('B4:C4')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('212529');
            $sheet->getStyle('B4:C4')->getFont()->getColor()->setARGB('FFFFFF');
            ...
            {
                $sheet->getStyle('B'.$index.':C'.$index)->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);
                $sheet->getStyle('B'.$index.':C'.$index)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_LEFT);
                $sheet->getStyle('C'.$index)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_RIGHT);
                $sheet->getStyle('C'.$index)->getNumberFormat()->setFormatCode('#,##0');

                $sheet->getStyle('B'.$index.':C'.$index)->applyFromArray($borderStyle);

                $sheet->setCellValue('B'.$index, $item['fecha']);
                $sheet->setCellValue('C'.$index, $item['valor']);
            ...
            $sheet->getStyle('B'.$index.':C'.$index)->getFont()->setBold(true)->setSize(12); 
            ...
            $sheet->setCellValue('C'.$index, $valor);
       
            $sheet->getSheetView()->setZoomScale(90);
            $sheet->setTitle('Inf entradas con');
        
            $writer = new Xlsx($spreadsheet);
            $temp_file = tempnam(sys_get_temp_dir(), 'InfEntradasConsol.xlsx');
            $writer->save($temp_file);
            
            return $this->file($temp_file, 'InfEntradasConsol.xlsx', ResponseHeaderBag::DISPOSITION_INLINE);
        }
        elseif($tipoInforme == 2)
        {
            $data = $bd->getRepository(Movimientos::class)->dataInforme2($desdeInforme, $hastaInforme);
            ...
            $sheet->getStyle('B1')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
            $sheet->getStyle('B1')->getFont()->setBold(true)->setSize(12);
            $sheet->setCellValue('B1', 'INFORME DE ENTRADAS CONSOLIDADO POR SERVICIO');
            $sheet->mergeCells('B2:C2');
            $sheet->getStyle('B2')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
            $sheet->getStyle('B2')->getFont()->setSize(12);
            $sheet->setCellValue('B2', 'Desde '.$desdeInforme.' | Hasta '.$hastaInforme);
            ...
            foreach($data as $item)
            {
                ...
                $sheet->setCellValue('B'.$index, $item['tipo']);
                $sheet->setCellValue('C'.$index, $item['valor']);
        ...
        elseif($tipoInforme == 3)
        {
            ...
            $sheet->getStyle('B1')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
            $sheet->getStyle('B1')->getFont()->setBold(true)->setSize(12);
            $sheet->setCellValue('B1', 'INFORME DE GASTOS CONSOLIDADO');
            $sheet->mergeCells('B2:C2');
            $sheet->getStyle('B2')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
            $sheet->getStyle('B2')->getFont()->setSize(12);
            $sheet->setCellValue('B2', 'Desde '.$desdeInforme.' | Hasta '.$hastaInforme);
            ...
            $sheet->getStyle('B'.$index)->getFont()->setBold(true)->setSize(12); 
            $sheet->getStyle('B'.$index)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_RIGHT);   
            $sheet->getStyle('C'.$index)->getNumberFormat()->setFormatCode('#,##0');        
            $sheet->setCellValue('B'.$index, 'TOTAL');
            $sheet->setCellValue('C'.$index, $valor);
            ...
            $sheet->getSheetView()->setZoomScale(90);
            $sheet->setTitle('Inf gastos con');
        
            $writer = new Xlsx($spreadsheet);
            $temp_file = tempnam(sys_get_temp_dir(), 'InfGastosConsol.xlsx');
            $writer->save($temp_file);
            
            return $this->file($temp_file, 'InfGastosConsol.xlsx', ResponseHeaderBag::DISPOSITION_INLINE);
        ...
        elseif($tipoInforme == 4)
        {
            ...
            $sheet->getStyle('B1')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
            $sheet->getStyle('B1')->getFont()->setBold(true)->setSize(12);
            $sheet->setCellValue('B1', 'INFORME DE GASTOS DETALLADO');
            ...
            $sheet->getStyle('B4:E4')->getFont()->setBold(true)->setSize(12);
            $sheet->getStyle('B4:E4')->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);
            ...
            $sheet->getStyle('B4:E4')->applyFromArray($borderStyle);
            $sheet->getStyle('B4:E4')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('212529');
            $sheet->getStyle('B4:E4')->getFont()->getColor()->setARGB('FFFFFF');
        

            $sheet->setCellValue('B4', 'FECHA');
            $sheet->setCellValue('C4', 'TIPO');
            $sheet->setCellValue('D4', 'DETALLE');
            ...
            foreach($data as $item)
            {
                $sheet->getStyle('B'.$index.':E'.$index)->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);
                $sheet->getStyle('B'.$index.':E'.$index)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_LEFT);
                $sheet->getStyle('E'.$index)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_RIGHT);
                $sheet->getStyle('E'.$index)->getNumberFormat()->setFormatCode('#,##0');

                $sheet->getStyle('B'.$index.':E'.$index)->applyFromArray($borderStyle);

                $sheet->setCellValue('B'.$index, $item['fechacrea']);
                $sheet->setCellValue('C'.$index, $item['tipo']);
                $sheet->setCellValue('D'.$index, $item['detalles']);
                $sheet->setCellValue('E'.$index, $item['valor']);
            ...
            $sheet->getStyle('E'.$index)->getNumberFormat()->setFormatCode('#,##0');        
            $sheet->setCellValue('B'.$index, 'TOTAL');
            $sheet->setCellValue('E'.$index, $valor);
       
            $sheet->getSheetView()->setZoomScale(90);
            $sheet->setTitle('Inf gastos det');
        
            $writer = new Xlsx($spreadsheet);
            $temp_file = tempnam(sys_get_temp_dir(), 'InfGastosDet.xlsx');
            $writer->save($temp_file);
            
            return $this->file($temp_file, 'InfGastosDet.xlsx', ResponseHeaderBag::DISPOSITION_INLINE);
        }
    }
